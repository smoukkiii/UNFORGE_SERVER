...
class InvTransactionsTest {
    ...
    @Test
    fun GameTestState.`equipment instance survives inventory and worn swaps`() =
        runBasicGameTest {
            withPlayerInit {
                ...
                assertEquals(instanceId, inv.filterNotNull { it.instanceId == instanceId }.single().instanceId)
            }
        }
    ...
    fun GameTestState.`equipment instances never merge in bank`() = runBasicGameTest {
        withPlayerInit {
            ...
            assertEquals(2, bank.occupiedSpace())
            assertEquals(setOf(101L, 202L), bank.filterNotNull { it.instanceId != 0L }.map { it.instanceId }.toSet())
        }
    }
